...
def save_adapter(model, out_dir, do_zip: bool) -> None:
    """Write the LoRA adapter (+ optional submission.zip) to out_dir, applying the
    Nemotron-H lm_head key rename the Kaggle loader expects. Safe to call mid-training."""
    from safetensors.torch import load_file, save_file
    out_dir = Path(out_dir)
    out_dir.mkdir(parents=True, exist_ok=True)
    print(f"\n=== Save adapter to {out_dir} ===")
...
def main() -> None:
    args = parse_args()
    # --- distributed data-parallel (torchrun) setup ---
    ...
    IS_MAIN = RANK == 0
    if IS_DIST:
        os.environ["CUDA_VISIBLE_DEVICES"] = str(LOCAL_RANK)  # one GPU per rank
        # per-rank compile caches + cwd so the ranks don't clobber each other's
        # Unsloth/Triton compiled artifacts (they share the job's working dir).
        for _k in ("TRITON_CACHE_DIR", "TORCHINDUCTOR_CACHE_DIR"):
            if os.environ.get(_k):
                os.environ[_k] = os.path.join(os.environ[_k], f"rank{LOCAL_RANK}")
        _rcwd = os.path.join(os.getcwd(), f"rank{LOCAL_RANK}")
        os.makedirs(_rcwd, exist_ok=True)
        os.chdir(_rcwd)
    ...
    import torch
    torch.backends.cuda.enable_cudnn_sdp(False)  # avoid flaky cuDNN fused-MHA graph crash (mha_graph.execute is_good()==false)
    from unsloth import FastLanguageModel
    if IS_DIST:
        import torch.distributed as dist
        dist.init_process_group(backend="nccl")
        print(f"[rank {RANK}/{WORLD_SIZE}] NCCL init, CUDA_VISIBLE_DEVICES={os.environ.get('CUDA_VISIBLE_DEVICES')}")
    ...
    for batch_start in range(0, len(indices), args.batch_size):
        ...
        if IS_DIST:
            # consecutive shard per rank; keeps micro-batch boundaries aligned with
            # the single-GPU run so SUM-all-reduced grads == the full-batch grad.
            per = len(batch_indices) // WORLD_SIZE
            batch_indices = (batch_indices[RANK * per : (RANK + 1) * per]
                             if RANK < WORLD_SIZE - 1 else batch_indices[RANK * per:])
        ...
        n = len(batch)
        # GLOBAL accumulation count (= the single-GPU value), so each rank's
        # (loss / n_accum) + SUM all-reduce reproduces the full-batch gradient.
        n_accum = math.ceil(args.batch_size / args.micro_batch_size)
        total_loss_sum = 0.0
        ...
        for pg in optimizer.param_groups:
            pg["lr"] = lr
        if IS_DIST:
            # data-parallel: SUM the per-rank LoRA grads (each already divided by the
        ...
        # Checkpoint at each completed epoch (rank 0 only; the final epoch is saved
        # to output_dir below). Lets a long multi-epoch run yield per-epoch adapters.
        if step % steps_per_epoch == 0 and step < num_steps:
            if IS_MAIN:
                save_adapter(model, f"{output_dir}-epoch{step // steps_per_epoch}", args.zip_submission)
            if IS_DIST:
                dist.barrier()

    print(f"\nTraining complete. Peak VRAM: {torch.cuda.max_memory_allocated() / 1e9:.1f} GB")

    # Final epoch -> output_dir (rank 0 only; the canonical adapter + submission.zip).
    if IS_MAIN:
        save_adapter(model, output_dir, args.zip_submission)
    if IS_DIST:
        dist.barrier()
